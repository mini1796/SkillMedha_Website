...
async function loadCourseDetails(id) {
  try {
    ...
    if (course) {
      ...
      document.querySelector(".course-main-content").innerHTML =
...
function renderDetails(course) {
  ...
  if (instContainer && course.courseInstructors) {
    ...
    course.courseInstructors.forEach((inst) => {
      instContainer.innerHTML += `
                <div class="instructor-badge" style="display:flex; align-items:center; gap:8px; margin-bottom:8px; padding: 8px 12px; background-color: #f1f5f9; border-radius: 6px; cursor: pointer; transition: background 0.2s;">
    ...
    if (totalSections > maxVisibleSections) {
      ...
      showMoreSectionsBtn.addEventListener("click", () => {
        const hiddenSections =
          accordionContainer.querySelectorAll(".hidden-section");
    ...
    couponBtn.addEventListener("click", (e) => {
      ...
      if (isCouponApplied) {
        couponMsg.textContent =
          "Coupon already applied! You've successfully locked in your discount.";
      ...
      if (enteredCode === DYNAMIC_VALID_COUPON) {
        ...
        if (DYNAMIC_DISCOUNT_AMOUNT > 0) {
          ...
          const currentPriceText = priceDisplay.textContent.replace(
            /[^0-9]/g,
            ""
          );
      ...
      couponMsg.textContent =
        "Sorry, that code is invalid or has expired. Please double-check the code and try again!";
      couponMsg.className = "coupon-msg error";
    });
  }
...
function initInstructorModal() {
  const modal = document.getElementById("instructorModal");
  const closeBtn = document.querySelector(".inst-close-btn");
  const instructorsContainer = document.getElementById("instructors-container");

  // Safety check: if elements don't exist, stop.
  if (!modal || !instructorsContainer) return;

  // 1. Event Delegation for Badge Clicks
  instructorsContainer.addEventListener("click", (e) => {
    // Find the closest .instructor-badge parent of the clicked element
    const badge = e.target.closest(".instructor-badge");

    if (badge) {
      const rawName = badge.innerText.trim(); // Use innerText to get just the name
      openInstructorModal(rawName, modal);
    }
  });
...
function openInstructorModal(nameString, modal) {
  // Mock Data Generator
  let name = nameString;
  ...
  // Auto-generate a colored avatar based on name
  const imageSrc = `https://ui-avatars.com/api/?name=${encodeURIComponent(
    name
  )}&background=27ae60&color=fff&size=200`;

  // Populate Modal
  const nameEl = document.getElementById("modalInstName");
  const roleEl = document.getElementById("modalInstRole");
  const bioEl = document.getElementById("modalInstBio");
  const imgEl = document.getElementById("modalInstImage");

  if (nameEl) nameEl.textContent = name;
  if (roleEl) roleEl.textContent = role;
  if (imgEl) imgEl.src = imageSrc;
  if (bioEl)
    bioEl.textContent = `${name} is a seasoned professional with over 10 years of experience in ${role}. They have mentored thousands of students and contributed to major industry projects.`;
